...
class Condition():
    ...
    def __init__(self, column, column_type, operator, value, btw_value=None):
        self.column = column
        self.column_type = column_type
        self.operator = operator
        self.value = value
        self.btw_value = btw_value
    ...
    def get_pretty_operator(self, operator):
        if operator == 'BETWEEN':
            return Color.BOLD + 'BETWEEN' + Color.END + ' ' + str(self.btw_value) + ' ' + Color.BOLD + 'AND' + Color.END
        else:
...
class Query():
    ...
    def __init__(self, select=None, _from=None, join=None, where=None, group_by=None, order_by=None, limit=None):
    ...
    def __str__(self):
        if len(self.limit) == 0 or self.limit == None:
            return '\n' + str(self.select) + str(self._from) + str(self.join) + str(self.where) + str(self.group_by) + str(
                self.order_by) + ';\n'
        else:
            return '\n' + str(self.select) + str(self._from) + str(self.join) + str(self.where) + str(
                self.group_by) + str(
                self.order_by) + Color.BOLD + ' LIMIT ' + Color.END + '{}'.format(self.limit[0]) + ';\n'
