...
class SelectParser(Thread):
    def __init__(self, columns_of_select, tables_of_from, phrase, count_keywords, sum_keywords, average_keywords,
                 max_keywords, min_keywords, distinct_keywords, database_dico, database_object, top_keywords):
    ...
    def run(self):
        for table_of_from in self.tables_of_from:  # for each query
            ...
            #     self.columns_of_select = [i.name for i in
            #                               self.database_object.get_column_names_by_table_name(table_of_from)]
            #     self.columns_of_select = self.uniquify(self.columns_of_select)
            #     self.phrase = self.columns_of_select
            #     number_of_select_column = len(self.columns_of_select)
                ...
                for i in range(0, len(self.phrase)):
                    for column_name in self.columns_of_select:
                        if (self.phrase[i] == column_name) or \
                                (self.phrase[i] in
                                 self.database_object.get_column_with_this_name(column_name).equivalences):
                ...
                for i in range(0, len(select_phrases)):
                    ...
                    for keyword in self.top_keywords:
                        if keyword in phrase:
                            idx = phrase.index(keyword)
                            num.append(phrase[idx+1])
                            self.select_object.add_num(num[0])
                            break
                    break
...
class WhereParser(Thread):
    def __init__(self, phrases, tables_of_from, columns_of_values_of_where, count_keywords, sum_keywords,
                 ...
                 equal_keywords, database_dico, database_object):
        Thread.__init__(self)
        self.where_objects = []
    ...
    def predict_operator(self, current_column_offset, next_column_offset):
        ...
        if (len(self.intersect(interval_offset, self.negation_keyword_offset)) >= 1) and (
                len(self.intersect(interval_offset, self.greater_keyword_offset)) >= 1) and (
                len(self.intersect(interval_offset, self.equal_keyword_offset)) >= 1):
            return '<='
        elif (len(self.intersect(interval_offset, self.negation_keyword_offset)) >= 1) and (
                len(self.intersect(interval_offset, self.less_keyword_offset)) >= 1) and (
                len(self.intersect(interval_offset, self.equal_keyword_offset)) >= 1):
            return '>='
        elif (len(self.intersect(interval_offset, self.negation_keyword_offset)) >= 1) and (
        ...
        if (len(self.intersect(interval_offset, self.less_keyword_offset)) >= 1) and (
                len(self.intersect(interval_offset, self.equal_keyword_offset)) >= 1):
            return '<='
        elif (len(self.intersect(interval_offset, self.greater_keyword_offset)) >= 1) and (
                len(self.intersect(interval_offset, self.equal_keyword_offset)) >= 1):
            return '>='
    ...
    def run(self):
        ...
        for table_of_from in self.tables_of_from:
            ...
            for i in range(0, len(column_offset)):
                ...
                if operator == "BETWEEN":
                    btw_value = self.columns_of_values_of_where[0]
                    where_object.add_condition(junction, Condition(column, operation_type, operator, value, btw_value))
                else:
...
class GroupByParser(Thread):
    ...
    def run(self):
        for table_of_from in self.tables_of_from:
            ...
            cols = []
            cols_alias = []
            for e, phrase in enumerate(self.phrases):
                for i in range(0, len(phrase)):
                    for table_name in self.database_dico:
                        columns = self.database_object.get_table_by_name(table_name).get_columns()
                        for column in columns:
                            if ((phrase[i] == column.name) or (phrase[i] in column.equivalences)) and (
                                    column.name not in cols):
                                column_with_alias = self.get_column_name_with_alias_table(column.name, table_of_from)
                                cols.append(column.name)
                                cols_alias.append(column_with_alias)
                                if e == len(self.phrases) - 1:
                                    r = ', '.join(cols_alias)
                                    group_by_object.set_column(r)
...
class Parser:
    ...
    def __init__(self, database, config):
        ...
        self.top_keywords = config.get_top_keywords()
    ...
    def parse_sentence(self, sentence, stopwordsFilter=None):
        ...
        if irext:
            irext = self.remove_accents(irext)
            ...
            assignment_list = self.equal_keywords + self.like_keywords + self.greater_keywords + self.less_keywords \
                              + self.negation_keywords + self.between_keywords
            ...
            maverickjoy_between_assigner = "*between*@3#>>*"
            ...
            for idx, assigner in enumerate(assignment_list):
                ...
                elif assigner in self.between_keywords:
                    assigner = str(" " + assigner + " ")
                    irext = irext.replace(assigner, str(" " + maverickjoy_between_assigner + " "))
                else:
                    assigner = str(" " + assigner + " ")
            ...
            for idx, x in enumerate(irext_list):
                ...
                if x == maverickjoy_between_assigner:
                    if index < len(irext_list) and irext_list[index] != maverickjoy_like_assigner and irext_list[
                        index] != \
                            maverickjoy_general_assigner:
                        # replace back <_> to spaces from the values assigned
                        columns_of_values_of_where.append(str("'" + str(irext_list[index]).replace('<_>', ' ') + "'"))
                        columns_of_values_of_where.append(str("'" + str(irext_list[index+2]).replace('<_>', ' ') + "'"))
        ...
        col = [i.name for i in self.database_object.get_column_names_by_table_name(tables_of_from[0])]
        rmvd = []
        if len(columns_of_select) != 0:
            columns_of_select = [i for i in columns_of_select if i in col]
            columns_of_select = list(dict.fromkeys(columns_of_select))
            select_phrase = list(dict.fromkeys(select_phrase))
        if len(columns_of_where) != 0:
            rmvd = list(set([i for i in columns_of_where if i not in col]))
            columns_of_where = [i for i in columns_of_where if i in col]
            if len(where_phrase) != 0:
                where_phrase = [i for i in where_phrase if i not in rmvd]
                for i in range(len(new_where_phrase)):
                    new_where_phrase[i] = [j for j in new_where_phrase[i] if j not in rmvd]
        for i in range(len(group_by_phrase)):
            group_by_phrase[i] = [j for j in group_by_phrase[i] if j in col or j in self.asc_keywords or j in self.desc_keywords]
            group_by_phrase[i] = list(dict.fromkeys(group_by_phrase[i]))
        for i in range(len(order_by_phrase)):
            order_by_phrase[i] = [j for j in order_by_phrase[i] if j in col or j in self.asc_keywords or j in self.desc_keywords]
        ...
        try:
            select_parser = SelectParser(columns_of_select, tables_of_from, select_phrase, self.count_keywords,
                                         ...
                                         self.distinct_keywords, self.database_dico, self.database_object, self.top_keywords)
            ...
            where_parser = WhereParser(new_where_phrase, tables_of_from, columns_of_values_of_where,
                                       ...
                                       self.equal_keywords, self.database_dico, self.database_object)
        ...
        for i in range(0, len(queries)):
            ...
            query.set_limit(select_objects[i].get_num())
